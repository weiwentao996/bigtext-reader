...
- **支持多种文本编码**：支持 UTF-8、GBK、GB18030、Big5、Shift_JIS、EUC-KR、Windows-1252 和自动检测。
- **流式全文搜索**：搜索开始后立即显示进度，命中结果会动态加载，并使用临时索引文件保存大量结果，避免结果过多时占满内存。
- **正则与大小写选项**：普通搜索、正则搜索、大小写敏感 / 不敏感搜索使用统一的进度与结果列表体验。
- **停止搜索**：搜索过程中可随时停止，已发现的结果仍可继续浏览、跳转和导出。
- **搜索结果虚拟滚动**：大量搜索结果也能无感滚动，不需要用户手动分页。
- **精确跳转搜索结果**：点击搜索结果可准确跳转到对应行和命中位置。
- **命中高亮**：阅读区和搜索预览中高亮关键词。
- **导出搜索结果**：可将搜索结果导出为 UTF-8 TSV 文件，方便用 Excel、WPS 或文本编辑器查看。
- **书签功能**：支持添加、跳转和删除书签。
- **阅读进度保存**：自动记录每个文件的阅读位置。
- **文件夹列表**：打开文件夹后可在侧边栏快速切换文件。
- **拖拽打开文件**：把文本文件拖入程序窗口即可在当前工作区打开。
- **单实例打开**：通过 Windows 打开文件时会复用已有窗口，避免弹出重复窗口。
- **编码热切换**：打开文件后也可以切换编码并刷新内容。
- **字号调节**：适合长时间阅读。
- **深色模式**：支持浅色 / 深色主题切换，首次启动会跟随系统偏好。
- **中英文界面**：内置中文和英文切换。
...
- 在 GB 级文本中搜索关键词。
- 打开中文、日文、韩文和常见旧西文编码文本。
- 记录长文本阅读进度和书签。
- 快速浏览一个文件夹内的大量日志或文本文件。
- 将文件拖入程序窗口，并保持在当前工作区阅读。
...
在整个文件中搜索关键词时会显示扫描进度，命中结果会边搜索边加载；搜索过程中可以停止，并继续浏览已发现结果或导出 TSV 表格。
...
搜索结果可导出为 UTF-8 TSV 文件。TSV 是使用 Tab 分隔列的文本表格，可用 Excel、WPS 表格、VS Code、Notepad++ 或其他文本编辑器打开。搜索未完成或被停止时导出的文件会标记 `InProgress` 或 `Canceled`，表示只包含当前已发现的结果。
...
从 [GitHub Releases](https://github.com/weiwentao996/bigtext-reader/releases) 下载最新版：

- Windows：`BigText-Reader-v1.0.1-windows-amd64.zip`

解压后运行 `BigText-Reader-v1.0.1-windows-amd64.exe`。

也可以把 `.txt`、`.log` 或其他纯文本文件拖入程序窗口直接打开。
...
- AI Agent 日志分析：通过接入 AI API，让工具能够自动总结日志、定位异常片段并给出排查建议。
- Skill 扩展机制：允许用户创建面向特定日志格式或业务场景的分析技能，复用常见排障流程。
- Windows 安装包。
- macOS 和 Linux 发布包。
